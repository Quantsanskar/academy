...
const Chem11Attendance = () => {
    ...
    const handleAttendanceSubmit = async (student) => {
        if (!student || !attendanceStatus || !absentDate) return;
        setLoading(true);
        setSubmit(false);
        try {
            // Submit attendance
            ...
            if (attendanceStatus === "absent") {
                await axios.post('http://127.0.0.1:8000/api/send-sms-request/', {
                    ...
                    message: `Your ward ${student.name} is absent on ${absentDate}.`
                });
            }
                ...
                {students.map((student) => (
                    <div key={student.id} className="student">
                        ...
                        <div className="buttons">
                            <button
                                className={`button ${attendanceStatus === 'present' && selectedStudent === student ? 'present' : ''}`}
                                onClick={() => { handleStudentSelect(student); handleStatusChange('present'); }}
                            >
                                Present
                            </button>
                            <button
                                className={`button ${attendanceStatus === 'absent' && selectedStudent === student ? 'absent' : ''}`}
                                onClick={() => { handleStudentSelect(student); handleStatusChange('absent'); }}
                            >
                                Absent
                            </button>
                            <input
                            ...
                            />
                            <button className="submit-button" onClick={() => handleAttendanceSubmit(student)}>
                                {loading ? <Spinner1 /> : 'Submit'}
                                {submit ? <Spinner2 /> : ''}
                            </button>
                        </div>
                    </div>
